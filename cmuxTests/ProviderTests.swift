...
final class ProviderAccountStoreRoundTripTests: XCTestCase {
    ...
    override func setUp() {
        ...
        store.keychainServiceResolver = { [testService] _ in testService! }
    }
    ...
    func testAddLoadUpdateRemoveRoundTrip() async throws {
        ...
        XCTAssertTrue(store.accounts.isEmpty)
        do {
            _ = try await store.secret(for: account.id)
    ...
    func testIndexPersistsAcrossInstances() async throws {
        ...
        reloaded.keychainServiceResolver = { [testService] _ in testService! }
